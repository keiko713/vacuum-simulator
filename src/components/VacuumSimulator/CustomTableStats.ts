...
import { TableData } from "./SampleTableStats";
...
export const getCustomTableStatsList = () => {
  ...
  const statsList: TableData[] = fromLocalStorage
    ? JSON.parse(fromLocalStorage)
    : [];
  return statsList;
};

export const isCustomTableName = (key: string) => {
  return getCustomTableStatsList()
    .map((table) => table.key)
    .includes(key);
};
